...
def parse_cols(cols: []):
    ...
    for col in cols:
        ...
        a_data = process_address(a_data, elements, positions)
        positions = [14, 15, 13, 18, 16, 19, 17]
        a_data = process_address(a_data, elements, positions)
        proccess_report(elements, r_data)

        dea = elements[0]
        b_name = elements[2]
        process_business(b_data, b_name, dea)
